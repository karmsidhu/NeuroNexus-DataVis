		...
		var view = {
			...
			pieGraph: {
				...
				generateChart: (data, width, height) => {
					...
					// 	.enter().append("path")
					// 	.attr("fill", function(d, i) { return view.pieGraph.color(i); })
					// 	.transition()
					// 	.duration(function(d, i) {return i * 800;})
			...
			pieGraph2: {
				generateChart: (data) => {
					...
					var width,
							height = 384,
							radius,
							color = view.pieGraph.color,
							svg = d3.select('#pieGraph')
											.append('svg')
											.attr('width', '100%');

					width = d3.select('#pieGraph').node().offsetWidth;
